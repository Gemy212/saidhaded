...
@section('content')
    ...
    <form action="{{ route('home') }}#portfolio" method="GET" class="mb-10 space-y-6">
        ...
        <div class="flex flex-col md:flex-row gap-4 items-center">
            ...
            <div class="relative w-full md:flex-grow">
                <input type="text" name="search" value="{{ $search ?? '' }}" placeholder="ابحث عن تصميم... (مثال: كلاسيكي، مصفح، قصر، نحاسي)" class="w-full bg-stone-900 border border-stone-800 focus:border-amber-500 focus:outline-none p-3.5 pr-10 text-stone-200 text-sm rounded-sm">
            ...
            <div class="flex gap-3 w-full md:w-auto">
                ...
                @if($search || ($category && $category !== 'all'))
                    <a href="{{ route('home') }}#portfolio" class="border border-stone-800 text-stone-400 px-6 py-3.5 font-semibold hover:bg-stone-900 transition text-sm text-center flex items-center justify-center">
                        إعادة تعيين
                    </a>
                @endif
            </div>
        ...
        <div class="flex flex-wrap gap-2 pt-2">
            ...
            @foreach($categories as $key => $label)
                ...
                <a href="{{ route('home', ['category' => $key, 'search' => $search]) }}#portfolio" 
                   class="px-4 py-2 text-xs font-semibold rounded-full border transition duration-300 {{ $currentCategory === $key ? 'bg-amber-600 text-stone-950 border-amber-600 font-bold' : 'border-stone-800 text-stone-400 hover:text-stone-200 hover:border-stone-700' }}">
                    {{ $label }}
                </a>
            @endforeach
        </div>
            ...
            <div class="bg-stone-900/50 border border-stone-800 p-6 flex flex-col justify-between hover:border-amber-600/30 transition duration-300">
                <div>
                    ...
                    @if($project->images && count($project->images) > 0)
                        ...
                        @endphp
                        <img src="{{ $imgUrl }}" alt="{{ $project->title }}" class="w-full h-56 object-cover mb-4 rounded-sm border border-stone-800/80">
                    @else
                    ...
                    <p class="text-stone-400 text-sm mb-4 leading-relaxed line-clamp-3">{{ $project->description }}</p>
                </div>
                <div>
                    <a href="/projects/{{ $project->id }}" class="text-amber-500 text-sm font-semibold hover:underline inline-block mt-4">عرض التفاصيل والمواصفات الفنية ←</a>
                </div>
            </div>
        @empty
